package frc.robot.Autonomous;

public class AutonConstants {
    public static final double ACCEPTABLE_DIST_METERS = 0.1;
    public static final double ACCEPTABLE_ANGLE_DEG = 2.0;
    public static final double ACCEPTABLE_STRICT_DIST_METERS = 0.05;
    public static final double ACCEPTABLE_STRICT_ANGLE_DEG = 1;

    public static final double RETURN_TIME_BUFFER = 5.0;
    public static final double TRAJ_GOAL_VELOCITY = 2.0;

    public static final double PRELOAD_SHOOTING_WAIT = 1.6;
    public static final double RETURN_FROM_COLLECTING_SHOOTING_WAIT = 2.3;
    public static final double OUTPOST_SCORING_WAIT = 15.0;

    public static final double ARTIFICIAL_PING_SEC = 0.15;
    public static final double DEPLOY_INTAKE_WAIT = 0.5;
}
