...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    DoublePressTracker.createTrigger(xboxDrv.view()).onTrue(new InstantCommand(() -> {
    ...
    // xboxAux.menu().onTrue(CatzFlywheels.Instance.setpointCommand(Setpoint.withVoltageSetpoint(3.5)));
    ...
    // xboxAux.view().multiPress(2, 0.4).onTrue(Commands.runOnce(()-> CatzSuperstructure.Instance.canResetPose = ! CatzSuperstructure.Instance.canResetPose));
    // xboxAux.povUpRight().onTrue(CatzSuperstructure.Instance.resetClimbPose());
    xboxAux.getHID().povDownLeft().onTrue(CatzSuperstructure.Instance.resetHoodPose());
    xboxAux.getHID().povUpLeft().onTrue(CatzSuperstructure.Instance.resetTurretPose());
    xboxAux.getHID().povDownRight().onTrue(CatzSuperstructure.Instance.resetDeployPose());
    // xboxAux.view().multiPress(2, 0.4).onTrue(CatzFlywheels.Instance.setpointCommand(Setpoint.withVoltageSetpoint(12.0)));
  }

  public static void rumbleDrv(double val) {
    xboxDrv.getHID().setRumble(RumbleType.LEFT_RUMBLE, val);
    xboxDrv.getHID().setRumble(RumbleType.RIGHT_RUMBLE, val);
  }
}
